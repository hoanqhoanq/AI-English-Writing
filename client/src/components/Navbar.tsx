...
export const Navbar: React.FC = () => {
  ...
  const handleQuickDemoLogin = async (role: 'learner' | 'admin') => {
    try {
      await quickLogin(role);
      setIsProfileOpen(false);
      navigate(role === 'admin' ? '/admin' : '/practice');
    } catch (err) {
      console.error(err);
    }
  };
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={`h-4 w-4 ${active ? (isAdminTab ? 'text-purple-700' : 'text-indigo-600') : (isAdminTab ? 'text-purple-600' : 'text-slate-400')}`} />
                  <span>{item.name}</span>
                </Link>
              );
            })}
        ...
        <div className="hidden md:flex items-center gap-3">
          {isAuthenticated && user ? (
            <>
              ...
              {/* Role / Level Badge */}
              {user.role === 'admin' ? (
                <span className="rounded-lg bg-purple-100 px-2.5 py-1 text-xs font-bold text-purple-800 border border-purple-200 flex items-center gap-1">
                  ...
                  <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl ring-1 ring-black/5 z-50">
                    <div className="border-b border-slate-100 px-3 py-2">
                      ...
                      <div className="mt-2 flex items-center justify-between text-xs text-slate-600 bg-slate-50 p-2 rounded-lg">
                        <span>Vai trò:</span>
                        <span className={`font-bold ${user.role === 'admin' ? 'text-purple-700' : 'text-indigo-600'}`}>
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => handleQuickDemoLogin('learner')}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-sm text-slate-700">
                  ...
                  <span className="rounded bg-indigo-50 px-2 py-0.5 text-xs font-bold text-indigo-700">
                    {user.role === 'admin' ? 'Quản trị viên' : `Trình độ: ${user.level}`}
                  </span>
                </div>
                <button
                  type="button"
