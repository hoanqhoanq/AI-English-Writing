...
import { WritingQuestion, Topic, GrammarTopic, AttemptRecord } from '../types';
...
export const QuestionBankPage: React.FC = () => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      // If endpoint returns recent attempts or overview
      const trendsRes = await api.get('/analytics/trends');
      // Also get attempts from memory / database
      const overviewRes = await api.get('/analytics/overview');
      if (overviewRes.data.success) {
        // Build mock history or fetch user attempts
      }
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/80 pb-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 flex items-center gap-2">
            ...
            <span>Thư viện câu hỏi & Lịch sử</span>
          </h1>
